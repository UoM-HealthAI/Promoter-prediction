...
# =============================================================================
    ...
    def EM(self, initialised_parameters, n_steps, n_traces, PERMITTED_MEMORY,
        ...
        logL_tot = np.full((1, n_steps), np.inf)
        ...
        log_likelihoods = np.full((1, n_traces), np.inf)
        for i_tr in np.arange(0, n_traces):
            log_likelihoods[0, i_tr] = np.inf
        logL_tot = np.full((1, n_steps), np.inf)
        ...
        #%%
                ...
                for g in gammas_cleaned: # HACK
                    g = {key:val for key, val in g.items() if val != np.inf} # HACK
                ...
                for m in np.arange(0,K):
                    for n in np.arange(0,K):
                        ...
                        v_M_input = v_M_input[v_M_input != np.inf]
